# SMALL Protocol v0.1 Specification

**SMALL** (State, Manifest, Artifact, Lineage, Lifecycle) is a protocol for durable, agent-legible project state. This specification defines version 0.1 of the protocol.

## Versioning

- **Protocol version**: `0.1`
- **Version format**: `small_version: "0.1"` (string, not number)
- All canonical artifacts MUST include `small_version: "0.1"` as a required field

## Canonical Artifacts

SMALL v0.1 defines exactly five canonical artifacts, all stored as YAML files in the `.small/` directory:
...
The following invariants are non-negotiable and MUST be enforced by all SMALL v0.1 implementations:
...
- All canonical artifacts MUST have `small_version: "0.1"` (exact string match)
- Version mismatches MUST cause validation to fail
- Future protocol versions will use different version strings

## Backwards Compatibility

- **v0.1 is the first version** - no backwards compatibility requirements
- Future versions (v0.2, v1.0, etc.) will maintain compatibility within major version
- Breaking changes will increment the major version number

## Extension Mechanism

- Non-canonical files may be placed under `.small/ext/` or ignored by v0.1 implementations
...
A SMALL v0.1 implementation MUST:
...
The reference CLI implementation (`cmd/small/`) demonstrates a complete SMALL v0.1 implementation and serves as the authoritative validator.
